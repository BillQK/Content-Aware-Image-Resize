...
{
    // Check if the row and col_start/end are valid
    assert(row >= 0 && row < m_height);
    assert(col_start >= 0 && col_start < m_width);
    assert(col_end >= 0 && col_end < m_width);
    assert(col_start <= col_end);

    // If col_end is greater than the width of the matrix, set it to the width
    if (col_end > m_width)
    {
        col_end = m_width;
    }

    // If col_start is less than zero, set it to zero
    if (col_start < 0)
    {
        col_start = 0;
    }
